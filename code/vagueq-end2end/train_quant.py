...
if __name__ == '__main__':
    ...
    tr_inp, tr_q_out = read_images(tr_inds, tr_size)
    v_inp, v_q_out = read_images(v_inds, v_size)
    learning_rate = 0.02
    nb_epochs = 1
    b_size = 85
    m = quant_model.QuantInc()
    model = m.build()
    filepath = "best_model/weight.new.hdf5"
    checkpoint = ModelCheckpoint(filepath, monitor = 'val_loss', verbose = 1, save_best_only = True, mode = 'min')
    callback_list = [checkpoint]
    hist = model.fit(tr_inp, tr_q_out, batch_size = b_size, epochs = nb_epochs, validation_data = (v_inp, v_q_out), callbacks = callback_list)
